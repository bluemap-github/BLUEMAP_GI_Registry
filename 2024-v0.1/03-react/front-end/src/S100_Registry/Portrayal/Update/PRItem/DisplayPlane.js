...
import { PUT_DISPLAY_PLANE } from '../../api/api';
    ...
    const handleSubmit = async () => {
        ...
        if (confirmed) {
            try {
                ...
                // PUT 요청 보내기 (API 경로를 적절히 설정해야 함)
                const response = await axios.put(PUT_DISPLAY_PLANE, bodyData, {
                    params: {
                ...
                <div>
                    ...
                    <div className="input-group input-group-sm mb-2" style={{ marginRight: '10px' }}>
                        ...
                        <input
                            type="number"
                            className="form-control"
                            name="order"  // 수정: order로 필드 이름 변경
                            value={formData.order || ""}  // formData에서 order 값 가져오기
                            onChange={handleChange}  // 수정: handleChange로 formData에 반영
                        />
                    </div>
