...
import { PUT_VIEWING_GROUP_LAYER } from '../../api/api';  // ViewingGroupLayer에 대한 PUT API 경로
...
const ViewingGroupLayer = ({ data, onClose }) => {
    ...
    const handleSubmit = async () => {
        ...
        if (confirmed) {
            try {
                ...
                // PUT 요청 보내기
                const response = await axios.put(PUT_VIEWING_GROUP_LAYER, bodyData, {
                    params: {
    ...
    return (
        <div>
            <h2>Update Viewing Group Layer</h2>
                ...
                <div>
                    ...
                    <div className="input-group input-group-sm mb-2" style={{ marginRight: '10px' }}>
                        ...
                        <input
                            ...
                            name="xmlID"  // xmlID를 formData의 일부로 처리
                            value={formData.xmlID || ""}
                            onChange={handleChange}  // handleChange로 formData에 반영
                        />
                    </div>
...
export default ViewingGroupLayer;
