...
const DinamicAssociationForm = ({ itemType, onFormSubmit }) => {
    ...
    const handleSelectChange = (itemName, value, isMultiple) => {
        let updatedData;
    
        if (isMultiple) {
            const selectedOptions = Array.from(value.selectedOptions);
            const selectedValues = selectedOptions.map((option) => {
                const [encrypted_data, iv] = option.value.split(",");
                return { encrypted_data, iv };
            });
            updatedData = {
                ...selectedData,
                [itemName]: selectedValues,  // 다중 선택 처리
            };
        } else {
            const [encrypted_data, iv] = value.split(",");
            updatedData = {
                ...selectedData,
                [itemName]: { encrypted_data, iv },  // 단일 선택 처리
            };
        }
