...
function Register() {
  ...
  const handleDetailClick = (item) => {
    ...
    navigate(`/${getDecryptedItem('REGISTRY_URI')}/iho-concept/detail`);  // 쿠키에서 REGISTRY_URI를 가져옴
  };
      ...
      <table className="table table-hover table-bordered" style={{ tableLayout: 'fixed', marginTop: '20px' }}>
        <thead>
          <tr className='table-success'>
            <th style={{ width: '15%', cursor: 'pointer', color: sortKey === 'name' ? 'green' : 'black' }} onClick={() => handleSortChange('name')}>
              Name {renderSortArrow('name')}
            </th>
            <th style={{ width: '15%', cursor: 'pointer', color: sortKey === 'camelCase' ? 'green' : 'black' }} onClick={() => handleSortChange('camelCase')}>
