...
const InnerNav = () => {
    ...
    // Get the first and second elements of the path split by '/'
    const getPathSegments = () => {
        ...
        if (secondSegment === 'mymain') {
            return [capitalizedSegment, 'View Registry List'];
        }

        return [capitalizedSegment, 'Defult'];
    };
    ...
    return (
        <div className='inner-nav'>
            <div style={{display: 'flex', alignItems: 'center'}}>
                <div className='inner-nav-link' style={{display: "flex", alignItems: "center"}} onClick={moveToMyPage}>
                ...
                <p className='inner-nav-link'>{secondSegment}</p>
            </div>
        </div>
    );
};
